...
const Field = ({ label, input }: FieldProps) => {
  return H(
    "div",
    { className: "uk-margin" },
    H("label", label, H("div", { className: "uk-form-controls" }, input)),
  );
};
...
const Toast = ({ message }: ToastProps) => {
  return H("div", { className: "uk-margin" }, message);
};
...
const bgColorTypeOptions: DropdownOption[] = [
  ...
  { text: "Gold", value: "FCD581" },
  { text: "Indigo", value: "9883E5" },
];
        ...
        H(ImagePreview, {
          ...
          onclick: (e: Event) => {
            ...
            if (success) {
              setState({
                showToast: true,
                messageToast: "Đã copy đường dẫn hình ảnh",
              });
